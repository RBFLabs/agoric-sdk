...
export const buildRootObject = async (vatPowers, vatParameters, baggage) => {
  ...
  const makeZcfThing1 = defineDurableKind(zcfHandle1, undefined, {});
  const makeZcfThing2 = defineDurableKind(zcfHandle2, undefined, {});

  const installationBaggage = provideBaggageSubset(baggage, 'installation');
  ...
  const { setupInstallation, setup } = contractNS;
  ...
  const doSetupInstance = async perInstanceZoeStuff => {
    ...
    const zcfThing2 = makeZcfThing1(perInstanceZoeStuff);
